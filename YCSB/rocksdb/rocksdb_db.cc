...
namespace {
  ...
  const std::string PROP_BLOOM_UNITS = "rocksdb.bloom_units";
  const std::string PROP_BLOOM_UNITS_DEFAULT = "0";
...
void RocksdbDB::GetOptions(const utils::Properties &props, rocksdb::Options *opt,
  ...
  if (options_file != "") {
    ...
    int bloom_units = std::stoul(props.GetProperty(PROP_BLOOM_UNITS, PROP_BLOOM_UNITS_DEFAULT));
    std::vector<int> bloom_unit_bits;
    if (bloom_bits > 0 && bloom_units > 0) {
      int unit_bits = bloom_bits / bloom_units;
      for (int i = 0; i < bloom_units; i++) { bloom_unit_bits.emplace_back(unit_bits); }
      std::cout << "filter units number: " << bloom_units << ", bits-per-key of one unit: " << unit_bits << std::endl;
      table_options.filter_policy.reset(rocksdb::NewBloomFilterPolicy(bloom_unit_bits, false));
      table_options.index_type = rocksdb::BlockBasedTableOptions::kTwoLevelIndexSearch;
      table_options.partition_filters = true;
    }
    table_options.bits_per_key_per_filter = bloom_unit_bits;
    // size_t metadata_cache_size = std::stoul(props.GetProperty(PROP_CACHE_SIZE, PROP_CACHE_SIZE_DEFAULT));
    table_options.metadata_cache = rocksdb::NewLRUCache(64*1024*1024);
    table_options.filter_info_cache = rocksdb::NewMultiQueue(64*1024*1024, bloom_unit_bits, 10000, 0.01);
