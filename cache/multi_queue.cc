...
uint64_t period_count = 1000000;
double curr_freq_weight = 0.6;
namespace rocksdb
{
    ...
    namespace multiqueue_ns
    {
        struct LRUQueueHandle
        {
            ...
            double fre_count;   //frequency count
        ...
        {
            std::cout << "use hotness instead of frequency" << std::endl;
        ...
        void MultiQueue::RecomputeExp(LRUQueueHandle *e)
        {
            ...
            uint64_t prev_period = (e->expire_time - life_time_) / period_count;
            uint64_t curr_period = (current_time_) / period_count;
            if (curr_period > prev_period) {
                for (uint64_t i = 0; i < curr_period - prev_period; i++) {
                    e->fre_count *= (1 - curr_freq_weight);
                }
            }
            e->fre_count += curr_freq_weight;
            sum_freqs_[e->queue_id] += e->fre_count;
            ...
            else
            {
                ...
                const double new_expection = expection_ - (e->fre_count - curr_freq_weight) * FalsePositive(e) + e->fre_count * fps[e->queue_id + 1]; //TODO: OPTIMIZE
